...
def scatterGraphData():
    ...
    for i in range (len(graphX)):   #labels scatterplot points with each team name
        plt.text(x = graphX[i] + 0.1, y = graphY[i] + 0.1, s = df.Tm[i], fontdict=dict(color = 'black', size = 10), bbox = dict(facecolor = 'yellow', alpha = 0.5))
...
def barGraphData():
    ...
    for i in range (len(graphX)):   #labels scatterplot points with each team name
        plt.text(x = graphX[i], y = graphY[i], s = df.Tm[i], fontdict=dict(color = 'black', size = 10), bbox = dict(facecolor = 'yellow', alpha = 0.5), rotation = 90)
